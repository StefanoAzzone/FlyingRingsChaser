...
uniform vec4 lightDir1;					//direction of the 3 lights
uniform vec4 lightDir2;
uniform vec4 lightDir3;
uniform vec4 matcol;					//color of the material
uniform vec3 eyePos;					//eye position

uniform vec4 shown;						//if complete render == [1,0,0,0]; if only Texture == [0,1,0,0]; Normals == [0,0,1,0],
										//UVs == [0,0,0,1], Environment == [0,0,0,0]
uniform vec4 effect;					//effect[i] == 1 if the effect is selected: Normal Map, Parallax Map, Ambient Occlusion, and Texture
uniform vec4 texSelect;					//arrays of 0 / 1 defining which texture to use
uniform vec4 monoSelect;
uniform vec4 repMetRough;				//[m,r,0,0], where m is Metalness and r Roughness.

uniform sampler2D u_tex_Albedo;			//sampler for the albedo texture
uniform sampler2D u_tex_NormalMap;		//sampler for the NormalMap texture
uniform sampler2D u_tex_RMAO;			//sampler for the "Roughness, Metalness and Ambient Occlusions" texture
uniform samplerCube u_tex_Env;			//sampler for the environment map texture
...
void main() {
	...
	mat3 tbn = mat3(t, b, n_norm);													//Matrix containing tangent, bitangent and normal
    ...
    vec2 texCoords = mix(fs_uv, ParallaxMapping(fs_uv,  viewDir), effect.g);		//effect.g is 1 if parallax mapping is selected;
																					//this means: use the normal UVs if ParMap is
	 																				//unchecked, parallax mapping's coordinates otherwise
	vec4 nMap = texture(u_tex_NormalMap, texCoords);								//Color of the normal map
	vec3 n = mix(n_norm, normalize(tbn * (nMap.xyz * 2.0 - 1.0)), effect.r);		//This means: use the "normal" normal ;) if NorMap is
								 													//unchecked, NormalMap's normal otherwise
	vec4 RMAO = texture(u_tex_RMAO, texCoords);										//Color representing Roughness, Metalness
																					//and Ambient Occlusions
	// Diffuse + ambient
	...
	dimFact *= mix(1.0, RMAO.b, effect.b);											//This means: if AmbOcl is selected multiply the
...
//	dimFact = dimFact * matcol.a + (1.0 - matcol.a);
	...
	// Phong specular
	float Rough = mix(RMAO.g, repMetRough.g, matcol.a);								//This means: if Texture is selected use as roughness
	...
	specFact += lightDir2.w * pow(max(dot(v, -reflect(lightDir2.xyz,n)),0.0), rf);	//This is to compute the Phong specular factor
	specFact += lightDir3.w * pow(max(dot(v, -reflect(lightDir3.xyz,n)),0.0), rf);	//without the environment contribute 
	...
	vec4 albedo = vec4(mix(texture(u_tex_Albedo, texCoords).rgb, matcol.rgb, matcol.a), 1.0);
																					//This means: if the material has alpha channel = 1,
																					//use the material's color as albedo, otherwise sample 
																					//from the albedo texture.

	vec4 texout = albedo;															//All this stuff means: depending on the texture
	texout = mix(texout, vec4(nMap.rgb, 1.0), texSelect.g);							//selected by the user use the actual albedo, the 
	vec4 mono = vec4(vec3(dot(vec4(RMAO.rgb, nMap.a), monoSelect)), 1.0);			//normalMap, Relief Map, Amb. Occ, Metallic, 
	texout = mix(texout, mono, texSelect.b);										//Roughness
	
	float Metal = mix(RMAO.r, repMetRough.r, matcol.a);								//If if the material has alpha channel = 1 use as
																					//Metalness the one selected by the user, otherwise
																					//sample from the RMAO texture

	vec4 diffColor = albedo * 0.96 * (1.0 - Metal);
	diffColor = mix(diffColor, vec4(1.0), ambEmuFact);								//WOW, He is using mix for an actual interpolation! OoO
	vec4 specColor = 1.0 + 0.96 * Metal * (albedo - 1.0);
	specColor = mix(specColor, vec4(1.0), ambEmuFact);								//same here
	...
	vec4 specFactFromEnvMap = textureLod(u_tex_Env, refDir, lod);					//This is the light of the environment that is
																					//reflected on the object

	vec4 selSpecFact = mix(specFactFromEnvMap * mix(0.4, 1.0, Metal), vec4(specFact), repMetRough.b);
																					//This is to compute the specular factor: the more 
																					//the object is metallic the more we consider the 
																					//environment color
	vec4 selDimFact = mix(textureLod(u_tex_Env, n, 8.0), vec4(dimFact), repMetRough.b);
																					//This is to compute the diffuse factor: the more 
																					//the object is rough the more we consider the lighting
																					//of the object with respect to the environment

	
//	color = mix(specFactFromEnvMap, texture(u_tex_Env, -v) ,repMetRough.a);
//	color = mix(specFactFromEnvMap, vec4(0.5,0.5,0.5,1.0) ,repMetRough.a);			//Compute the final color, based on the selected mode:
	color = mix(specFactFromEnvMap, mix(texture(u_tex_Env, -v), vec4(0.5,0.5,0.5,1.0), repMetRough.b) ,repMetRough.a);
																					//if we want only the environment we use only its color,
																					//(considering Roughnessa and Metalness).
	color = mix(color, vec4((diffColor * selDimFact + specColor * selSpecFact).rgb, 1.0), shown.r);
																					//If we are using full render, we consider the
																					//specular and diffusion components.
	color = mix(color, texout, shown.g);											//If we use only the texture, we only consider its raw value
	color = mix(color, vec4(0.5+0.5*n, 1.0), shown.b);								//This is if we want to show only the normals.
	color = mix(color, vec4(0.5+0.5*(10.0*texCoords-9.0*fs_uv), 0.0, 1.0), shown.a);
																					//This is if we want to show only the UVs.
}`;
