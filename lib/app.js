	...
	for (i = 0; i < asteroids.length; i++) {
		...
		for(var j = 0; j < lasers.length; j++) {
			...
			if(norma(dx) < laserDist && norma(dy) < laserDist && norma(dz) < laserDist) {
				...
				var sx = norma(asteroids[i].xSpeed) >= 0.02 ? asteroids[i].xSpeed * 7 : 0.1;
				var sy = norma(asteroids[i].ySpeed) >= 0.02 ? asteroids[i].ySpeed * 7 : 0.1;
...
function explosion(t) {
	...
	if(dt != lastExpTime) {
		r = 4 - Math.floor(dt / 5);
		c = dt % 5;
		translate = utils.MakeTranslateMatrix(c/5,r/5,0.0);
		scale = utils.MakeScaleMatrix(1/5);
