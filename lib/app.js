...
async function loadObjects() {
	...
	await loadObject("obj/spaceship-cockpit.obj");
...
function drawShip() {
	...
	worldMatrix = utils.MakeWorld(cx, cy-0.5, cz, shipElevation, shipAngle - 180, 0.0, 1.0);
	setMatrices();
	drawMesh();
...
function spawnAsteroid() {
	...
	if(!lastAsteroidSpawn || (currentTime - lastAsteroidSpawn >= asteroidSpawnCooldown)) {
		...
		asteroids.push(new Asteroid(ax, ay, sx, sy, s, rx, ry, rz, true));
