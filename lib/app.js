...
async function loadObjects() {
	skyboxVao = await loadObject("obj/square.obj");
	cockpitVao = await loadObject("obj/cockpit.obj");
	ringVao = await loadObject("obj/ring.obj");
	laserVao = await loadObject("obj/laser.obj");
	asteroidVao = await loadObject("obj/asteroid.obj");
	//skyboxVao = await loadObject("obj/rock2.obj");
	//skyboxVao = await loadObject("obj/rock3.obj");
	//skyboxVao = await loadObject("obj/rock4.obj");
	//skyboxVao = await loadObject("obj/quad.obj");
}
...
function drawSkybox() {
	...
	gl.uniform1i(program.cubemap, 19);
...
function drawShip() {
	gl.bindVertexArray(cockpitVao);
	gl.uniform4fv(program.effectsUniform, [0.0,1.0,0.0,0.0]);
	setObjectSamplers(0);
...
function drawAsteroids() {
	...
	for(i = 0; i < asteroids.length; i++) {
		if (asteroids[i].active) {
			...
			worldMatrix = utils.multiplyMatrices(utils.MakeTranslateMatrix(asteroids[i].x, asteroids[i].y, asteroids[i].z), utils.MakeScaleMatrix(asteroids[i].size));
			setMatrices();
			var mat = explosion(asteroids[i].explosionAnimTime);
			gl.uniform1f(program.animationUniform, 1.0);
			gl.uniformMatrix4fv(program.animMatUniform, gl.FALSE, mat);
			//drawMesh(mesh + asteroidMeshNum);
		}
	}
	if(asteroids.length > maxAsteroidCount)
		asteroids.shift();
	mesh++;
}
...
function getDeltaT() {
	...
	delta = (currentTime - lastUpdateTime)/1000;
	return delta
}
...
function drawLasers() {
	...
	for(i = 0; i < lasers.length; i++) {
		if (lasers[i].active) {
			worldMatrix = utils.MakeWorld(lasers[i].pos[0] + 2, lasers[i].pos[1] - 1, lasers[i].pos[2], elevation, angle, 0, 1);
			setMatrices();
			drawMesh(mesh);
...
const maxRingCount = 10;
const maxAsteroidCount = 25;
...
function spawnAsteroid() {
	var currentTime = getCurrentTime();
	if(!lastAsteroidSpawn || (currentTime - lastAsteroidSpawn >= asteroidSpawnCooldown)) {
		...
		asteroids.push(new Asteroid(ax, ay, sx, sy, s, rx, ry, rz, type, true));
	}
}
...
function spawnLaser() {
	...
	dir = utils.multiplyMatrixVector(rot, dir).splice(0,3);
...
function checkAsteroidCollision() {
	for (i = 0; i < asteroids.length; i++) {
		...
		size += 4 * (asteroids[i].size - 0.2) * (1 / 0.7);
		...
		for(var j = 0; j < lasers.length; j++) {
			...
			if(norma(dx) < laserDist && norma(dy) < laserDist && norma(dz) < laserDist) {
				asteroids[i].active = false;
				lasers.splice(j, 1);
				break;
			}
	...
	if(!dead) {
		if(gameTime < fadingTime) {
			...
			if(gameTime < loadingTime && gameTime > 1000) {
				...
				gl2.fillText(message, midWidth - message.length/2*28, midHeight - 20);
				message = 'AVOID ROCKS';
				gl2.fillText(message, midWidth - message.length/2*28, midHeight + 80);
			}

		}
		else {
			if(start == false && getGameTime() > 15)
				start = true;
			gl2.fillStyle = 'green';
...
function explosion(t) {
	var dt = Math.floor((t / 3.0) * 16);
	dt = 16 - dt;
	r = Math.floor(dt / 4);
	c = dt % 4;
	translate = utils.MakeTranslateMatrix(c/4,r/4,0.0);
	scale = utils.MakeScaleMatrix(1/4);
...
async function loadObject(path) {
	...
	vertices = objModel.vertices; //Array of vertices
	normals = objModel.vertexNormals; //Array of normals
	uvs = objModel.textures; //Array of uvcoordinates
	indices = objModel.indices; //Array of indices

	totMesh ++;
	startIndex[totMesh] = startIndex[totMesh-1] + indices.length;
	startVertex[totMesh] = startVertex[totMesh-1] + vertices.length;
