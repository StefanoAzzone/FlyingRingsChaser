...
function write() {
	...
	if(!dead)
		...
		var deathMessage = 'THE ALIENS GOT TO YOU';
...
async function main(){
	...
	if(gl){
		...
		await loadObject("obj/X-Wing4.obj");
...
function deathAnimation() {
	cx = cx + Math.random()*10;
	cy = cy + Math.random()*10;
	cz = cz + Math.random()*10;
...
function collisionDetection() {
	...
	for (i = 0; i < ringCoord.length; i++) {
		if (ringCoord[i][2] > ringDetectionInterval[0] && ringCoord[i][2] < ringDetectionInterval[1] && 
													distanceFromShip(ringCoord[i][0], ringCoord[i][1]) < innerRadius && activeRings[i]) {
