...
function write() {
	...
	gl2.font = '48px serif';
	gl2.fillStyle = 'white';
	gl2.fillText('RINGS: ' + collectedRings + '/' + totRings, 10, 50);
}
...
function deathAnimation() {
	elevation = elevation*updateShipDirectionSpeed + Math.random()*10*(1-updateShipDirectionSpeed);
	angle = angle*updateShipDirectionSpeed + Math.random()*10*(1-updateShipDirectionSpeed);
}
...
function drawScene() {
	...
	worldMatrix = utils.MakeWorld(cx, cy, cz, shipElevation, shipAngle -90.0, 0.0, 1.0);
	...
	if((new Date).getTime() - lastAsteroidSpawn >= asteroidSpawnCooldown) {
		for (i = 0; i < activeAsteroids.length; i++) {
			if (!activeAsteroids[i]) {
				...
				sx = (Math.random() - 0.1) * speed * 0.4 * (cx - rx) * 0.02;
				sy = (Math.random() - 0.1) * speed * 0.4 * (cy - ry) * 0.02;
...
function collisionDetection() {
	//Rings
	for (i = 0; i < ringCoord.length; i++) {
		if (ringCoord[i][2] > ringDetectionInterval[0] && ringCoord[i][2] < ringDetectionInterval[1] && 
													distanceFromShip(ringCoord[i]) < innerRadius && activeRings[i]) {
	...
	for (i = 0; i < asteroidCoord.length; i++) {
		...
		if (asteroidCoord[i][5] >= 0.07) size += 2;
		else if (asteroidCoord[i][5] <= 0.04) size -= 2;

		if (asteroidCoord[i][2] > asteroidDetectionInterval[0] && asteroidCoord[i][2] < asteroidDetectionInterval[1] && 
													distanceFromShip(asteroidCoord[i]) < size && activeAsteroids[i]) {
			
			
			console.log("Ouch, that hurt!!!\nNext time remember to stay more than " + distanceFromShip(asteroidCoord[i])
...
function distanceFromShip(obj) {
	return Math.sqrt(Math.pow((cx - obj[0]),2) + Math.pow((cy - obj[1]),2));
}
