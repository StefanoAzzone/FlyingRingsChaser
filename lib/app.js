...
function drawScene() {
	...
	worldMatrix = utils.MakeWorld(cx, cy, cz, shipElevation, shipAngle - 90, 0.0, 1.0);
	...
	for(i = 0; i < ringCoord.length; i++) {
		worldMatrix = utils.MakeWorld(ringCoord[i][0], ringCoord[i][1], ringCoord[i][2], 100.0, 0.0, 0.0, 1.0);
		viewWorldMatrix = utils.multiplyMatrices(viewMatrix, worldMatrix);
		wvpMatrix = utils.multiplyMatrices(projectionMatrix, viewWorldMatrix);
		gl.uniformMatrix4fv(program.WVPmatrixUniform, gl.FALSE, utils.transposeMatrix(wvpMatrix));
		//Rings
