...
function drawScene() {
	...
	spotLightDir = [0.0, 0.0, 1.0];
	spotLightColor = [1.0, 1.0, 1];
	spotLightPos = [cx, cy, cz, 1.0];
	mesh = 0;
...
function drawSkybox() {
    ...
    //var spotLightPosTransformed2 = [spotLightPosTransformed[0],spotLightPosTransformed[1],spotLightPosTransformed[2]];
	// draws the request
	...
	gl.uniform1f(program.coneInUniform, 80);
	gl.uniform1f(program.coneOutUniform, 30);
	...
	gl.uniform3fv(program.effectsUniform, [1.0,0.0,0.0]);
...
function drawShip() {
	...
	gl.uniform3fv(program.effectsUniform, [0.0,0.0,0.0]);
	//Ship
	gl.drawElements(gl.TRIANGLES, startIndex[mesh+1] - startIndex[mesh],
		gl.UNSIGNED_SHORT, startIndex[mesh]*2);
