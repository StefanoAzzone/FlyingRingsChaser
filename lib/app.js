...
function animate(){
	...
	if(lastUpdateTime){
		...
		var i;
		for (i = 0; i < ringCoord.length; i++) {
			ringCoord[i][2] += deltaC * speed * Math.cos(utils.degToRad(elevation)) * Math.cos(utils.degToRad(angle));
			
			if (ringCoord[i][2] > despawnDistance) spawn(i);
		}
		if (i < maxRingCount) {
			spawn(i);
		}
