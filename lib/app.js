...
function setDirectionalLight() {
	var directionalLightDir = [0.0, -1.0, 0.25];
...
function drawAsteroids() {
	gl.uniform4fv(program.effectsUniform, [0,1,1,0]);
	setObjectSamplers();
...
function drawLasers() {
	gl.uniform4fv(program.effectsUniform, [0,0,0,1]);
...
function boostShip(delta) {
	if(boost) {
		...
		if(newEnergy >= 0) {
			shipSpeed = (shipSpeed + 0.1 * delta) < boostFactor ? (shipSpeed + 0.1 * delta) : boostFactor;
			energy = newEnergy;
		}
		else {
			energy = 0;
